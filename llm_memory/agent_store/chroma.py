...
class ChromaStorageConnector(StorageConnector):
    ...
    def __init__(self, table_type: str, config: MemGPTConfig, user_id, agent_id=None):
        ...
        assert (
            table_type == TableType.ARCHIVAL_MEMORY or table_type == TableType.PASSAGES
        ), "Chroma only supports archival memory"
    ...
    def get_all_paginated(
        self, filters: Optional[Dict] = {}, page_size: int = 1000, offset: int = 0
        ...
        while True:
            ...
            assert results["embeddings"] is not None, "results['embeddings'] was None"
    ...
    def results_to_records(self, results) -> List[RecordType]:
        ...
        if results["embeddings"]:  # may not be returned, depending on table type
            ...
            return [
                ...
                for (text, id, metadatas) in zip(
                    results["documents"], results["ids"], results["metadatas"]
                )
            ]
    ...
    def get_all(self, filters: Optional[Dict] = {}, limit=None) -> List[RecordType]:
        ...
        if limit:
            results = self.collection.get(
                ids=ids, include=self.include, where=filters, limit=limit
            )
        else:
    ...
    def size(self, filters: Optional[Dict] = {}) -> int:
        # unfortunately, need to use pagination to get filtering
