...
from typing import Type, Union, Optional, Dict, Tuple, List, Iterator
from data_types import Record
from config import MemGPTConfig
from data_types import Passage, Message, Document, RecordType
...
class TableType:
    ...
    RECALL_MEMORY = (
        "recall_memory"  # archival memory table: memgpt_agent_recall_{agent_id}
    )
...
class StorageConnector:
    ...
    def __init__(
        self,
        table_type: Union[
            TableType.ARCHIVAL_MEMORY,
            TableType.RECALL_MEMORY,
            TableType.PASSAGES,
            TableType.DOCUMENTS,
        ],
        config: MemGPTConfig,
        ...
        if (
            ...
            or self.table_type == TableType.RECALL_MEMORY
        ):
            # agent-specific table
            assert (
                agent_id is not None
            ), "Agent ID must be provided for agent-specific tables"
            self.filters = {"user_id": self.user_id, "agent_id": self.agent_id}
        elif (
            self.table_type == TableType.PASSAGES
            or self.table_type == TableType.DOCUMENTS
        ):
            # setup base filters for user-specific tables
            assert (
                agent_id is None
            ), "Agent ID must not be provided for user-specific tables"
    ...
    def get_archival_storage_connector(user_id, agent_id):
        ...
        return StorageConnector.get_storage_connector(
            TableType.ARCHIVAL_MEMORY, config, user_id, agent_id
        )
    ...
    def get_recall_storage_connector(user_id, agent_id):
        config = MemGPTConfig.load()
        return StorageConnector.get_storage_connector(
            TableType.RECALL_MEMORY, config, user_id, agent_id
        )

    @abstractmethod
    def get_filters(
        self, filters: Optional[Dict] = {}
    ) -> Union[Tuple[list, dict], dict]:
        pass

    @abstractmethod
    def get_all_paginated(
        self, filters: Optional[Dict] = {}, page_size: int = 1000
