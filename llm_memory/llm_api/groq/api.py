...
def get_groq_completion(
    ...
    try:
        response = post_json_auth_request(
            uri=URI, json_payload=request, auth_type=auth_type, auth_key=auth_key
        )
        if response.status_code == 200:
            ...
            if "context length" in str(response.text).lower():
                ...
                raise Exception(
                    f"Request exceeds maximum context length (code={response.status_code}, msg={response.text}, URI={URI})"
                )
            else:
                raise Exception(
    ...
    completion_tokens = (
        usage.get("completion_tokens", None) if usage is not None else None
    )
    total_tokens = (
